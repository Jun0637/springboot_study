...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        .usernameParameter("memberid")		//loginForm(로그인폼)의 아이디 입력란의 name
        .passwordParameter("memberpw")		//loginForm(로그인폼)의 비밀번호 입력란의 name
        .and()
        .logout()
    ...
    public void configure(AuthenticationManagerBuilder auth) throws Exception {
        ...
        // 인증 (로그인)을 했나 안했나
        //** 이거 꼭 맞춰줘야함. 로그인폼html이랑. username, password, enabled
        .usersByUsernameQuery(
        		"select memberid username, memberpw password, enabled " +
                "from spring5_member " +
                "where memberid = ?")
        // 권한 로그인되었어도 그럴 권한이 있나
